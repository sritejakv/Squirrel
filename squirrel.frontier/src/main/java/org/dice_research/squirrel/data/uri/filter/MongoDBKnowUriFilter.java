...
public class MongoDBKnowUriFilter implements KnownUriFilter, Cloneable{
	...
	public boolean isUriGood(CrawleableUri uri) {
		MongoCursor<Document> cursor = mongoDB.getCollection(COLLECTION_NAME).find(new Document("uri",uri.getUri().toString())).iterator();
	...
	public void add(CrawleableUri uri, long timestamp) {
		mongoDB.getCollection(COLLECTION_NAME).insertOne(crawleableUriToMongoDocument(uri).append("timestamp", timestamp));
	...
	public Document crawleableUriToMongoDocument(CrawleableUri uri) {
        ...
        return new Document("ipAddress", ipAddress.toString())
        		.append("type", uriType.toString())
        		.append("uri", uri.getUri().toString());
